...
#define XInputGetState SystemXInputGetState
#define XInputSetState SystemXInputSetState
#include <xinput.h>
...
static decltype(&SystemXInputGetState) g_realGetState = nullptr;
static decltype(&SystemXInputSetState) g_realSetState = nullptr;
...
static DWORD WINAPI WorkerThread(void*)
{
    ...
    while (true)
    {
        if (GetAsyncKeyState(VK_F6) & 1) RebaseTime(1.30);
        if (GetAsyncKeyState(VK_F7) & 1) RebaseTime(1.50);
        if (GetAsyncKeyState(VK_F8) & 1) RebaseTime(2.00);
        Sleep(15);
    }
}
